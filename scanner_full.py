...
def run_scan_cycle(symbols: list, now_time: int, alerted_today: dict, momentum_today: dict,
    ...
    ts           = datetime.now(TZ_VN).strftime('%H:%M:%S')
    print(f"  [{ts}] Bắt đầu update {len(cache_symbol_set)} mã, quét {len(symbols)} mã (VNDirect đa luồng)...")

    # Tải song song nến hôm nay (VNDirect 8 luồng)
    today_bars_map = {}
    with ThreadPoolExecutor(max_workers=8) as executor:
        futures = {executor.submit(fetch_today_bar, s, current_date): s for s in cache_symbol_set}
        for fut in futures:
            s = futures[fut]
            try:
                bar = fut.result()
                if bar is not None:
                    today_bars_map[s] = bar
            except Exception:
                pass
    ...
    for symbol in cache_symbol_set:
        try:
            ...
            today_bar = today_bars_map.get(symbol)
