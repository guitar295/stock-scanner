...
.hv-sym-item { display:flex;align-items:center;padding:5px 6px;cursor:pointer;border-bottom:1px solid rgba(0,0,0,.04);transition:background .1s;gap:6px; }
...
.hv-sym-pct { font-family:var(--font-mono);font-size:10px; }
...
function _hoverCell(sym){
  ...
  _hoverPreviewTimer = setTimeout(()=>{
    ...
    document.querySelectorAll('.hv-sym-item').forEach(el => el.classList.toggle('on', el.dataset.sym === sym));
    _hvScrollActiveIntoView();
  }, 600); // Thay 180 thành 1000 (1 giây) để tránh nháy chart khi di chuyển chuột nhanh
}
