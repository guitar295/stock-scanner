...
function render(data,ts){
  ...
  stockNodes.forEach(stock=>{
    ...
    flows.forEach((flow,idx)=>{
      ...
      svg.appendChild(makeEl('rect',{x:chart.stockX,y:segY,width:chart.barW,height:segH,rx:(idx===0||idx===flows.length-1)?2:0,fill:sec?sec.color:'#94a3b8', 'shape-rendering':'crispEdges'}));
      segY+=segH;
    });
    if(h2>6){
