      ...
      <div class="tpanel on" id="panel-tv">
        ...
        <div id="tv-chart-wrap" style="display:none">
          ...
          <div id="tv-legend">
            ...
            <span class="tv-leg-item"><span class="tv-leg-dot" style="background:#1a56db"></span><span class="tv-leg-label">EMA10</span><span class="tv-leg-val" id="leg-ema10" style="color:#1a56db">—</span></span>
            <span class="tv-leg-item"><span class="tv-leg-dot" style="background:#f59e0b"></span><span class="tv-leg-label">EMA20</span><span class="tv-leg-val" id="leg-ema20" style="color:#f59e0b">—</span></span>
            <span class="tv-leg-item"><span class="tv-leg-dot" style="background:#8b5cf6"></span><span class="tv-leg-label">EMA50</span><span class="tv-leg-val" id="leg-ema50" style="color:#8b5cf6">—</span></span>
            <span class="tv-leg-item"><span class="tv-leg-dot" style="background:#ef4444;height:2px;width:14px"></span><span class="tv-leg-label">MA200</span><span class="tv-leg-val" id="leg-ma200" style="color:#ef4444">—</span></span>
          ...
          <div id="tv-macd-container">
            <div id="tv-macd-legend">
              ...
              <span class="tv-leg-item"><span class="tv-leg-label" style="font-size:9px">Signal</span><span class="tv-leg-val" id="leg-signal" style="color:#f97316;font-size:10px">—</span></span>
...
<!-- TradingView Lightweight Charts — serve local, không cần internet -->
...
const TV_COLORS={
  ...
  macdHistDn: 'rgba(224,36,36,0.7)',
  volUp:  'rgba(14,159,110,0.45)',
...
async function loadTVChart(sym){
  ...
  try{
    ...
    _tvChart=LightweightCharts.createChart(mainEl,{
      ...
      rightPriceScale:{borderColor:'#e5e7eb',scaleMarginTop:0.08,scaleMarginBottom:0.08},
      timeScale:{
        borderColor:'#e5e7eb',
        rightOffset:50,        // lề phải 50 bar
        fixLeftEdge:false,
        fixRightEdge:false,
        timeVisible:true,
    ...
    // Volume (histogram phụ, dùng HistogramSeries trên price scale khác)
    ...
    _tvVolSeries.setData(d.volume||[]);

    // EMAs
    if(d.ema10&&d.ema10.length){const s=_tvChart.addLineSeries({color:TV_COLORS.ema10,lineWidth:1,priceLineVisible:false,lastValueVisible:false,crosshairMarkerVisible:false});s.setData(d.ema10);}
    if(d.ema20&&d.ema20.length){const s=_tvChart.addLineSeries({color:TV_COLORS.ema20,lineWidth:1.5,priceLineVisible:false,lastValueVisible:false,crosshairMarkerVisible:false});s.setData(d.ema20);}
    if(d.ema50&&d.ema50.length){const s=_tvChart.addLineSeries({color:TV_COLORS.ema50,lineWidth:1.5,priceLineVisible:false,lastValueVisible:false,crosshairMarkerVisible:false});s.setData(d.ema50);}
    if(d.ma200&&d.ma200.length){const s=_tvChart.addLineSeries({color:TV_COLORS.ma200,lineWidth:2,lineStyle:LightweightCharts.LineStyle.Dashed,priceLineVisible:false,lastValueVisible:false,crosshairMarkerVisible:false});s.setData(d.ma200);}
    ...
    _tvMacdChart=LightweightCharts.createChart(macdEl,{
      ...
      timeScale:{
        borderColor:'#e5e7eb',
        rightOffset:50,        // đồng bộ lề phải với main chart
        timeVisible:false,
      },
      handleScroll:true,
      handleScale:true,
    ...
    _tvChart.subscribeCrosshairMove(param=>{
      if(!param.time||!_tvMacdChart) return;
      _tvMacdChart.setCrosshairPosition(param.point?.x??0,0,_tvHistSeries);
      // Cập nhật legend
    ...
    const lastEma10=d.ema10?.at(-1);if(lastEma10) document.getElementById('leg-ema10').textContent=lastEma10.value?.toFixed(2)||'—';
    const lastEma20=d.ema20?.at(-1);if(lastEma20) document.getElementById('leg-ema20').textContent=lastEma20.value?.toFixed(2)||'—';
    const lastEma50=d.ema50?.at(-1);if(lastEma50) document.getElementById('leg-ema50').textContent=lastEma50.value?.toFixed(2)||'—';
    const lastMa200=d.ma200?.at(-1);if(lastMa200) document.getElementById('leg-ma200').textContent=lastMa200.value?.toFixed(2)||'—';
    ...
    if(lastHist){
      ...
      hEl.style.color=lastHist.value>=0?TV_COLORS.macdHistUp.replace('0.7','1'):TV_COLORS.macdHistDn.replace('0.7','1');
    }
